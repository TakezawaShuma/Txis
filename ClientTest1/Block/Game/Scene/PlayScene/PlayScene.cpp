...
PlayScene::PlayScene(ISceneChange* _change) :
	m_player(nullptr), m_otherPlayers(map<SOCKET, OtherPlayer*>()), m_delList(list<SOCKET>()),
	...
	m_effect(nullptr), m_damegeEffectFlag(false), m_uiManager(nullptr)
{
	...
	if (m_player != nullptr) { m_player->Start(); }
}
...
bool PlayScene::Update(DX::StepTimer const& _timer)
{
	...
	if (m_player != nullptr)
	{
		if (m_player->HP() > 0)
		{
			...
			// その他プレイヤーの更新
			for (map<SOCKET, OtherPlayer*>::iterator it = m_otherPlayers.begin(); it != m_otherPlayers.end(); it++) 
			{ it->second->TextureUpdate(m_player->GetPosition()); }
...
void PlayScene::Draw()
{
	...
	if (m_uiManager != nullptr&&m_player!=nullptr)	m_uiManager->Render(UIType::HP, m_player->HP());
...
{
	_data;
	Data::Pakets::PlayData p;
	memcpy(&p, _data, sizeof(Data::Pakets::PlayData));
	if (NetworkClient::GetInstance().FindOthers(p.socket))
	{
		if (p.socket == NetworkClient::GetInstance().GetMySocket())
		{
			...
			{
				// プレイヤーの獲得
				m_player = new Player(p.socket, Vector2(p.posX, p.posY), p.hitPoint, p.scene.nowScene);
		...
		else
		{
			...
			{
				if (p.scene.nextScene != Data::Scenes::SCENETYPE::BACK) { m_otherPlayers[p.socket]->Update(p); }
