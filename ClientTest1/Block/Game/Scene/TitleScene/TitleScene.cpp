...
void TitleScene::Start(System::EffectManager* _effect)
{
	// エフェクトの取得と設定
	m_effect = _effect;
	m_dataSize = 0;
...
bool TitleScene::Update(DX::StepTimer const& _timer)
{
	...
	m_effect->Update(timer, Data::Scenes::SCENETYPE::TITLE, m_effectType);
	...
	{
		// 入力準備
		if (System::InputManager::GetInstance().GetKeyTracker().pressed.Enter)
		{
			m_inputIpFlag = true;
			return true;
		}
		// デバック用(ロビーへ移動)
		else if (System::InputManager::GetInstance().GetKeyTracker().pressed.L)
		{
			m_changeScene->ChangeScene(Data::Scenes::SCENETYPE::LOBBY);
			return true;
		}
		// デバック用(プレイシーンへ移動)
		else if (System::InputManager::GetInstance().GetKeyTracker().pressed.P)
		{
			m_changeScene->ChangeScene(Data::Scenes::SCENETYPE::PLAY);
			return true;
		}
		// デバック用(リザルトシーンへ移動)
		else if (System::InputManager::GetInstance().GetKeyTracker().pressed.R)
		{
			m_changeScene->ChangeScene(Data::Scenes::SCENETYPE::RESULT);
			return true;
		}
	}
	// IP入力フラグが立っているなら
	else
	{
		...
		if (System::InputManager::GetInstance().GetKeyTracker().pressed.Enter)
		{
			...
			if (m_ip == "0000")
			{
				...
				m_changeScene->ChangeScene(Data::Scenes::SCENETYPE::LOGIN);
...
void TitleScene::Draw()
{
	...
	if (m_effect != nullptr)
	{
		m_effect->Render(Data::Scenes::SCENETYPE::TITLE);
	}
	// 入力ボックスや文字の描画
	if (m_inputIpFlag)
	{
		System::DrawManager::GetInstance().Draw(*m_inputBox);
		...
		for (u_int i = 0; i < m_IpNum.size(); i++)
		{
			...
			System::DrawManager::GetInstance().Draw(*m_numbers);
		}
	}
}
